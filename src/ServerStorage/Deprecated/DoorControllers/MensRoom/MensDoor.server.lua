...
local door = workspace.BathroomMens
...
game.Workspace.BathroomMens.ClickDetector.MouseClick:Connect(function(player)
	if open == false then
		open = true
		game.ReplicatedStorage.Doors.MensRoom.MensDoor:FireAllClients(DoorSwingInfo, properties)
		door.DoorOpen:Play()
		wait(3)
		game.ReplicatedStorage.Doors.MensRoom.MensDoor:FireAllClients(closeInfo, closeProerties)
		wait(1.1)
		door.DoorClose:Play()
		wait(0.65)
		open = false
