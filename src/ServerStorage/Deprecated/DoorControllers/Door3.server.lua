...
local door = workspace:FindFirstChild("BackstageDoor")
...
game.Workspace:WaitForChild("BackstageDoor").ClickDetector.MouseClick:Connect(function(player)
	if player:GetRankInGroup(4890641) >= 0 then
		if door:GetAttribute("open") == false then
			door:SetAttribute("open", true)
			game.ReplicatedStorage.Doors.Door3:FireClient(player, DoorSwingInfo, properties)
			warn("fired opening")

			door.DoorOpen:Play()
			wait(3)
			game.ReplicatedStorage.Doors.Door3:FireClient(player, closeInfo, closeProerties)
			warn("fired closing")
			wait(1.1)
			door.DoorClose:Play()
			wait(0.65)
